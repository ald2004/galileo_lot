...
std::string read_all_data(mraa::Uart* uart) {
	...
	uart->flush();
	while(uart->dataAvailable())uart->readStr(1);
	uart->flush();
...
int main(void)
{
	...
	while (flag) {
	    ...
	    */
		...
		sprintf(formatted, "CH4:%.5f|CH2O:%2.4f mg/m3", float_value, fin_result);
		...
		if (now % 10 == 0) {
			if (!tag) {
				lcd.clear();
				lcd.setCursor(0, 0);
				//CH4:0.03226 | CH2O : 0.1212 mg / m3
				char oled_ch4[20];
				sprintf(oled_ch4, "CH4:%.5f", float_value);
				lcd.setCursor(2, 0);
				lcd.write(oled_ch4);
				lcd.setCursor(3, 0);
				sprintf(oled_ch4, "CH2O:%2.4fmg/m3", fin_result);
				lcd.write(oled_ch4);
				tag = !tag;
				lcd.setCursor(0, 0);
			}
